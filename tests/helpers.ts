...
export function makeTestApp(aiClient: AiClient | null = null) {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'card-shop-test-'));
  const created = createApp({ dataDir, adminPassword: PASSWORD, sessionSecret: 'x'.repeat(64), aiClient, webDir: null });
